...
def compute_window_table(ecg_arr: np.ndarray, ppg_arr: np.ndarray,
    ...
    # Elgendi 2016 in batch with auto-polarity detection (some devices like
    # Empatica E4 report PPG inverted relative to fingertip conventions)
    from ..signals.elgendi_sqi import batch_elgendi
    elg_df = batch_elgendi(ppg_arr[:n], ppg_fs, auto_invert=True)

    log.info("Computing per-window HR, Sukor SQI, Elgendi SQI on %d windows…", n)
    for i in range(n):
        ...
        row = {
            ...
            "elgendi_acceptable": int(elg_df["elgendi_acceptable"].iloc[i]),
            "elgendi_skewness": float(elg_df["elgendi_skewness"].iloc[i]),
            "elgendi_kurtosis": float(elg_df["elgendi_kurtosis"].iloc[i]),
            "elgendi_entropy": float(elg_df["elgendi_entropy"].iloc[i]),
        }
        rows.append(row)
...
class FourWaySQIAgreement:
    ...
    kappa_inhouse_vs_orph: float
    kappa_inhouse_vs_sukor: float
    kappa_inhouse_vs_elgendi: float
    kappa_orph_vs_sukor: float
    kappa_orph_vs_elgendi: float
    kappa_sukor_vs_elgendi: float
    median_kappa_published_only: float
    median_kappa_inhouse_vs_published: float
    inhouse_pass_rate: float
    orphanidou_pass_rate: float
    sukor_pass_rate: float
    elgendi_pass_rate: float
    fraction_all_published_pass: float
    fraction_all_published_fail: float
    inhouse_passes_when_all_published_fail: float


def four_way_sqi_agreement(df: pd.DataFrame,
                           inhouse_threshold: float = 0.7) -> FourWaySQIAgreement:
    """Compute pairwise kappas across four PPG SQIs and the headline
    'in-house outlier' statistic.
    ...
    """
    empty = FourWaySQIAgreement(
        n_windows=0,
        kappa_inhouse_vs_orph=float("nan"),
        kappa_inhouse_vs_sukor=float("nan"),
        kappa_inhouse_vs_elgendi=float("nan"),
        kappa_orph_vs_sukor=float("nan"),
        kappa_orph_vs_elgendi=float("nan"),
        kappa_sukor_vs_elgendi=float("nan"),
        median_kappa_published_only=float("nan"),
        median_kappa_inhouse_vs_published=float("nan"),
        inhouse_pass_rate=float("nan"),
        orphanidou_pass_rate=float("nan"),
        sukor_pass_rate=float("nan"),
        elgendi_pass_rate=float("nan"),
        fraction_all_published_pass=float("nan"),
        fraction_all_published_fail=float("nan"),
        inhouse_passes_when_all_published_fail=float("nan"),
    )
    if df is None or df.empty:
        return empty
    required = {"inhouse_ppg_sqi", "orphanidou_acceptable",
                "sukor_acceptable", "elgendi_acceptable"}
    if not required.issubset(df.columns):
        return empty
    ...
    if len(sub) < 5:
        empty.n_windows = len(sub)
        return empty
    in_h = (sub["inhouse_ppg_sqi"] >= inhouse_threshold).astype(int).to_numpy()
    orph = sub["orphanidou_acceptable"].astype(int).to_numpy()
    suk = sub["sukor_acceptable"].astype(int).to_numpy()
    elg = sub["elgendi_acceptable"].astype(int).to_numpy()

    k_io = cohens_kappa(in_h, orph)
    k_is = cohens_kappa(in_h, suk)
    k_ie = cohens_kappa(in_h, elg)
    k_os = cohens_kappa(orph, suk)
    k_oe = cohens_kappa(orph, elg)
    k_se = cohens_kappa(suk, elg)
    published_kappas = [k for k in (k_os, k_oe, k_se) if k == k]
    inhouse_kappas = [k for k in (k_io, k_is, k_ie) if k == k]
    return FourWaySQIAgreement(
        n_windows=len(sub),
        kappa_inhouse_vs_orph=k_io,
        kappa_inhouse_vs_sukor=k_is,
        kappa_inhouse_vs_elgendi=k_ie,
        kappa_orph_vs_sukor=k_os,
        kappa_orph_vs_elgendi=k_oe,
        kappa_sukor_vs_elgendi=k_se,
        median_kappa_published_only=float(np.median(published_kappas))
                                      if published_kappas else float("nan"),
        median_kappa_inhouse_vs_published=float(np.median(inhouse_kappas))
                                           if inhouse_kappas else float("nan"),
        inhouse_pass_rate=float(in_h.mean()),
        orphanidou_pass_rate=float(orph.mean()),
        sukor_pass_rate=float(suk.mean()),
        elgendi_pass_rate=float(elg.mean()),
        fraction_all_published_pass=float(
